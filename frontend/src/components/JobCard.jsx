...
  const [isCalendarMessage, setIsCalendarMessage] = useState(false);
...
    setIsLoading(true);
    setMessage("Saving your internship...");
...
     
      setMessage(data.success ? "Job saved successfully!" : data.message || "Failed to save job.");
    } catch (err) {
...
    if (isSaved) {
      setIsCalendarMessage(true);
      setTimeout(() => setIsCalendarMessage(false), 3000);
      return;
    }

    setIsLoading(true);
    setMessage("Adding to calendar...");
...
      const data = await res.json();
     
      setMessage("Added to calendar & saved!");
    } catch (err) {
      setMessage("Could not save job to your profile. Try again.");
    } finally {
      setTimeout(() => {
        setIsLoading(false);
        setMessage("");
...
            <p className="mt-4 text-gray-900 font-medium text-lg pixelify-sans-regular">
              {message}
            </p>
          </div>
        </div>
...
        <div className="fixed inset-0 flex items-center justify-center bg-gray-800/80 z-50">
          <div className="flex flex-col items-center p-6 bg-white rounded-lg shadow-xl">
            <p className="text-gray-900 font-medium text-lg pixelify-sans-regular">
              Job is already saved!
            </p>
          </div>
        </div>
      )}
      {/* Already Saved Message for Calendar */}
      {isCalendarMessage && (
        <div className="fixed inset-0 flex items-center justify-center bg-gray-800/80 z-50">
          <div className="flex flex-col items-center p-6 bg-white rounded-lg shadow-xl">
            <p className="text-gray-900 font-medium text-lg pixelify-sans-regular">
